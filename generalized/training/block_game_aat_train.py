...
from generalized.agents.algaater import ESTIMATES_LOOKBACK, TRAIN_RANDOM_PROB, TRAIN_RANDOM_N_ROUNDS_RATIOS, \
    distance_func, AssumptionChecker, Assumptions, Algaater
...
def create_opponent_agents(player_idx):
    experts = Algaater.create_aat_experts(block_game, player_idx)
    opponents = {}

    for agent_name, agent in experts.items():
        new_agent_name = agent_name + 'Opp'
        opponents[new_agent_name] = deepcopy(agent)
...
for use_random_switching in [True, False]:
    for epoch in range(1, n_epochs + 1):
        ...
        assumption_checker = AssumptionChecker(block_game, algaater_idx, baselines)
...
experts = AssumptionChecker(block_game, P1, baselines).experts
