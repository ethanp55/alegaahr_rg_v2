...
REWARDS = [[(1, -1), (-1, 1)],
           [(-1, 1), (1, -1)]]
baselines = {'AlgaaterCoop': 0, 'AlgaaterCoopPunish': 0, 'AlgaaterBully': 1, 'AlgaaterBullyPunish': 1,
             'AlgaaterBullied': -1, 'AlgaaterMinimax': -1, 'AlgaaterCfr': 0}


class PenniesGameState(GameState):
    ...
    def __eq__(self, other):
        if isinstance(other, PenniesGameState):
...
class PenniesGameMDP(MarkovGameMDP):
    def __init__(self):
        state = PenniesGameState()
