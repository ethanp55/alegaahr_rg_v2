...
algaater_compressed = pd.read_csv('./prionsers_dilemma_game_stoch/algaater_compressed.csv')
...
bbl_compressed = pd.read_csv('./prionsers_dilemma_game_stoch/bbl_compressed.csv')
bbl_compressed['Agent'] = ['BBL'] * len(bbl_compressed)
bbl_compressed.rename(columns={'BBL': 'Rewards'}, inplace=True)

eee_compressed = pd.read_csv('./prionsers_dilemma_game_stoch/eee_compressed.csv')
eee_compressed['Agent'] = ['EEE'] * len(eee_compressed)
eee_compressed.rename(columns={'EEE': 'Rewards'}, inplace=True)

spp_compressed = pd.read_csv('./prionsers_dilemma_game_stoch/spp_compressed.csv')
...
combined_df = pd.concat([algaater_compressed, bbl_compressed, eee_compressed, spp_compressed], axis=0,
                        ignore_index=True)
combined_df.to_csv('./prionsers_dilemma_game_stoch/combined_compressed.csv')
...
plt.savefig(f'./prionsers_dilemma_game_stoch/agent_rewards.png', bbox_inches='tight')
