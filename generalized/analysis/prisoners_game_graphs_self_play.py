...
algaater_compressed = pd.read_csv('./prionsers_dilemma_game/algaater_self_play.csv')
...
bbl_compressed = pd.read_csv('./prionsers_dilemma_game/bbl_self_play.csv')
bbl_compressed['Agent'] = ['BBL'] * len(bbl_compressed)
bbl_compressed.rename(columns={'BBL': 'Rewards'}, inplace=True)

eee_compressed = pd.read_csv('./prionsers_dilemma_game/eee_self_play.csv')
eee_compressed['Agent'] = ['EEE'] * len(eee_compressed)
eee_compressed.rename(columns={'EEE': 'Rewards'}, inplace=True)

spp_compressed = pd.read_csv('./prionsers_dilemma_game/spp_self_play.csv')
...
combined_df = pd.concat([algaater_compressed, bbl_compressed, eee_compressed, spp_compressed], axis=0,
                        ignore_index=True)
combined_df.to_csv('./prionsers_dilemma_game/combined_self_play.csv')
...
plt.savefig(f'./prionsers_dilemma_game/agent_rewards_self_play.png', bbox_inches='tight')
plt.clf()
