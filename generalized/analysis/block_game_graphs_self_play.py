...
algaater_compressed = pd.read_csv('./block_game/algaater_self_play.csv')
...
combined_df = pd.concat([algaater_compressed, bbl_compressed, eee_compressed, spp_compressed], axis=0,
                        ignore_index=True)
combined_df.to_csv('./block_game/combined_self_play.csv')
...
plt.xlabel('Agent')
plt.ylabel('Rewards')
plt.title('Self Play Rewards - Block Game')
plt.savefig(f'./block_game/agent_rewards_self_play.png', bbox_inches='tight')
plt.clf()
