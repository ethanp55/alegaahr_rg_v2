...
algaater_compressed = pd.read_csv('./pennies_game/algaater_compressed.csv')
...
combined_df = pd.concat([algaater_compressed, bbl_compressed, eee_compressed, spp_compressed], axis=0,
                        ignore_index=True)
combined_df.to_csv('./pennies_game/combined_compressed.csv')
...
plt.savefig(f'./pennies_game/agent_rewards.png', bbox_inches='tight')
plt.clf()

algaater_compressed_test = pd.read_csv('./pennies_game/algaater_compressed_test.csv')
...
combined_df_test.to_csv('./pennies_game/combined_compressed_test.csv')
...
plt.ylabel('Rewards')
plt.title('Agent Rewards - Matching Pennies')
plt.savefig(f'./pennies_game/agent_rewards_test.png', bbox_inches='tight')
plt.clf()

algaater_compressed_test = pd.read_csv('./pennies_game/algaater_compressed_test_changers.csv')
algaater_compressed_test['Agent'] = ['Algaater'] * len(algaater_compressed_test)
algaater_compressed_test.rename(columns={'Algaater': 'Rewards'}, inplace=True)

bbl_compressed_test = pd.read_csv('./pennies_game/bbl_compressed_test_changers.csv')
bbl_compressed_test['Agent'] = ['BBL'] * len(bbl_compressed_test)
bbl_compressed_test.rename(columns={'BBL': 'Rewards'}, inplace=True)

eee_compressed_test = pd.read_csv('./pennies_game/eee_compressed_test_changers.csv')
eee_compressed_test['Agent'] = ['EEE'] * len(eee_compressed_test)
eee_compressed_test.rename(columns={'EEE': 'Rewards'}, inplace=True)

spp_compressed_test = pd.read_csv('./pennies_game/spp_compressed_test_changers.csv')
spp_compressed_test['Agent'] = ['S++'] * len(spp_compressed_test)
spp_compressed_test.rename(columns={'S++': 'Rewards'}, inplace=True)

combined_df_test = pd.concat([algaater_compressed_test, bbl_compressed_test, eee_compressed_test, spp_compressed_test],
                             axis=0, ignore_index=True)
combined_df_test.to_csv('./pennies_game/combined_compressed_test_changers.csv')
combined_df_test.reset_index(drop=True, inplace=True)
...
plt.ylabel('Rewards')
plt.title('Agent Rewards - Matching Pennies')
plt.savefig(f'./pennies_game/agent_rewards_test_changers.png', bbox_inches='tight')
plt.clf()
