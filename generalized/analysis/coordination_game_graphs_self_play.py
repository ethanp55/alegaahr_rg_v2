...
algaater_compressed = pd.read_csv('./coordination_game/algaater_self_play.csv')
...
combined_df = pd.concat([algaater_compressed, bbl_compressed, eee_compressed, spp_compressed], axis=0,
                        ignore_index=True)
combined_df.to_csv('./coordination_game/combined_self_play.csv')
...
plt.xlabel('Agent')
plt.ylabel('Rewards')
plt.title('Self Play Rewards - Coordination Game')
plt.savefig(f'./coordination_game/agent_rewards_self_play.png', bbox_inches='tight')
plt.clf()
