...
for epoch in range(1, n_epochs + 1):
    ...
    spp = SPP('SPP', coordination_game, spp_idx)
    bbl = BBL('BBL', coordination_game, bbl_idx)
...
compressed_rewards_df.to_csv(f'../analysis/{str(coordination_game)}/bbl_vs_spp_bbl.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['S++'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(coordination_game)}/bbl_vs_spp_spp.csv')
...
plt.title('BBL vs. S++ - Coordination Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(coordination_game)}/bbl_vs_spp.png', bbox_inches='tight')
plt.clf()
