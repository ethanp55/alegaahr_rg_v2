...
compressed_rewards_df.to_csv(f'../analysis/{str(pennies_game)}/algaater_vs_bbl_algaater.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(pennies_game)}/algaater_vs_bbl_bbl.csv')
...
plt.title('AlgAATer vs. BBL - Matching Pennies')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(pennies_game)}/algaater_vs_bbl.png', bbox_inches='tight')
plt.clf()
