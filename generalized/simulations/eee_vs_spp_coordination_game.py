...
compressed_rewards_df.to_csv(f'../analysis/{str(coordination_game)}/eee_vs_spp_eee.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['S++'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(coordination_game)}/eee_vs_spp_spp.csv')
...
plt.title('EEE vs. S++ - Coordination Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(coordination_game)}/eee_vs_spp.png', bbox_inches='tight')
plt.clf()
