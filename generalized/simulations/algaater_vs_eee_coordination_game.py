...
compressed_rewards_df.to_csv(f'../analysis/{str(coordination_game)}/algaater_vs_eee_algaater.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['EEE'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(coordination_game)}/algaater_vs_eee_eee.csv')
...
plt.title('AlgAATer vs. EEE - Coordination Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(coordination_game)}/algaater_vs_eee.png', bbox_inches='tight')
plt.clf()
