...
for epoch in range(1, n_epochs + 1):
    ...
    algaater = Algaater('Algaater1', block_game, algaater_idx, baselines)
        ...
        for round_num in range(n_rounds):
            ...
            while not state.is_terminal():
                for agent_key, agent in key_agent_map.items():
                    ...
                    if agent_key == opponent_key and state.turn == opponent_idx:
                        opp_action = agent_action1 if opponent_agent.player == P1 else agent_action2
                        opp_actions.append(opp_action)

                    elif agent_key == algaater.name and state.turn == algaater_idx:
            ...
            algaater_rewards.append(agent_reward / 100)
...
if save_data:
    ...
    compressed_rewards_df.to_csv(f'../analysis/{str(block_game)}/algaater_compressed.csv')
    compressed_rewards_test_df.to_csv(f'../analysis/{str(block_game)}/algaater_compressed_test.csv')
    compressed_rewards_test_changers_df.to_csv(f'../analysis/{str(block_game)}/algaater_compressed_test_changers.csv')
    full_rewards_df.to_csv(f'../analysis/{str(block_game)}/algaater_full.csv')
    full_rewards_test_df.to_csv(f'../analysis/{str(block_game)}/algaater_full_test.csv')
    full_rewards_test_changers_df.to_csv(f'../analysis/{str(block_game)}/algaater_full_test_changers.csv')
...
if create_graphs:
    for opponent_key in opponents.keys():
        ...
        plt.savefig(f'../simulations/{str(block_game)}/algaater_vs_{opponent_key}.png', bbox_inches='tight')
        plt.clf()
