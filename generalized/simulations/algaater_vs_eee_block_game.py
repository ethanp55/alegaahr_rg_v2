...
for epoch in range(1, n_epochs + 1):
    ...
    for round_num in range(n_rounds):
        ...
        epoch_rewards_2.append(reward_map[eee.name] / 100)
...
compressed_rewards_df.to_csv(f'../analysis/{str(block_game)}/algaater_vs_eee_algaater.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['EEE'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(block_game)}/algaater_vs_eee_eee.csv')
...
plt.title('AlgAATer vs. EEE - Block Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(block_game)}/algaater_vs_eee.png', bbox_inches='tight')
plt.clf()

print(f'AlgAATer 20 rounds: {mean_test_results[19]}')
print(f'EEE 20 rounds: {mean_opponent_results[19]}')
