...
for epoch in range(1, n_epochs + 1):
    ...
    bbl = BBL('BBL', pennies_game, bbl_idx)
    ...
    for round_num in range(n_rounds):
        ...
        epoch_rewards_1.append(reward_map[bbl.name])
...
compressed_rewards_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_vs_spp_bbl.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['S++'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(pennies_game)}/bbl_vs_spp_spp.csv')
...
plt.title('BBL vs. S++ - Matching Pennies')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(pennies_game)}/bbl_vs_spp.png', bbox_inches='tight')
plt.clf()
