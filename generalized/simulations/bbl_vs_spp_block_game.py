...
from generalized.agents.algaater import ESTIMATES_LOOKBACK
...
for epoch in range(1, n_epochs + 1):
    ...
    print('S++: ' + str(spp_idx))
    print('EEE: ' + str(bbl_idx))

    spp = SPP('SPP', block_game, spp_idx)
    bbl = BBL('BBL', block_game, bbl_idx)

    # n_rounds = np.random.choice(possible_rounds)
    n_rounds = min_rounds

    reward_map = {spp.name: 0, bbl.name: 0}
    ...
    for round_num in range(n_rounds):
        print(f'BBL expert: {bbl.expert_to_use.name}')
        print(f'S++ expert: {spp.expert_to_use}')
        ...
        key_agent_map = {spp.name: spp, bbl.name: bbl} if spp_idx == P1 else \
            {bbl.name: bbl, spp.name: spp}
        ...
        epoch_rewards_1.append(reward_map[bbl.name] / 100)
        prev_rewards_2.append(prev_reward_2)
        epoch_rewards_2.append(reward_map[spp.name] / 100)
        spp.update_actions_and_rewards(opp_actions_1, opp_actions_2, prev_reward_1)
        bbl.record_opp_actions(round_num, opp_actions_1)

        print(f'BBL reward: {prev_reward_2}')
        print(f'S++ reward: {prev_reward_1}')
...
compressed_rewards_df = pd.DataFrame(vals_1, columns=['BBL'])
compressed_rewards_df.to_csv(f'../analysis/{str(block_game)}/bbl_vs_spp_bbl.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['S++'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(block_game)}/bbl_vs_spp_spp.csv')
...
plt.title('BBL vs. S++ - Block Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(block_game)}/bbl_vs_spp.png', bbox_inches='tight')
plt.clf()

print(f'BBL 20 rounds: {mean_test_results[19]}')
print(f'S++ 20 rounds: {mean_opponent_results[19]}')
print(f'BBL: {mean_test_results[-1]}')
