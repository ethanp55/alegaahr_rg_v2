...
for epoch in range(1, n_epochs + 1):
    ...
    expert_agents_1 = Algaater.create_aat_experts(block_game, bbl_idx_1)
    expert_agents_2 = Algaater.create_aat_experts(block_game, bbl_idx_2)

    bbl_1 = BBL('BBL1', block_game, bbl_idx_1, n_samples=10)
    bbl_2 = BBL('BBL2', block_game, bbl_idx_2, n_samples=10)
    ...
    for round_num in range(n_rounds):
        ...
        epoch_rewards_1.append(reward_map[bbl_1.name] / 100)
        epoch_rewards_2.append(reward_map[bbl_2.name] / 100)
...
compressed_rewards_df.to_csv(f'../analysis/{str(block_game)}/bbl_self_play.csv')
...
plt.savefig(f'../simulations/{str(block_game)}/bbl_self_play.png', bbox_inches='tight')
plt.clf()

print(f'BBL1 20 rounds: {mean_test_results[19]}')
print(f'BBL2 20 rounds: {mean_opponent_results[19]}')
