...
from generalized.agents.chicken_game_specific_agents import Random, GreedyUntilNegative, \
...
def create_opponent_agents(player_idx):
    initial_state = chicken_game.get_init_state()
    ...
    coop_punish_agent = FolkEgalPunishAgent('CoopPunishOpp', coop_agent, game_name, chicken_game)
    ...
    bully_punish_agent = FolkEgalPunishAgent('BullyPunishOpp', bully_agent, game_name, chicken_game)
    ...
    eee_experts = Algaater.create_aat_experts(chicken_game, player_idx)
    spp = SPP('RoundNum3S++', chicken_game, player_idx)
...
for epoch in range(1, n_epochs + 1):
    ...
    bbl = BBL('BBL', chicken_game, bbl_idx, n_samples=10)
...
if save_data:
    ...
    compressed_rewards_df.to_csv(f'../analysis/{str(chicken_game)}/bbl_compressed.csv')
    compressed_rewards_test_df.to_csv(f'../analysis/{str(chicken_game)}/bbl_compressed_test.csv')
    compressed_rewards_test_changers_df.to_csv(f'../analysis/{str(chicken_game)}/bbl_compressed_test_changers.csv')
    full_rewards_df.to_csv(f'../analysis/{str(chicken_game)}/bbl_full.csv')
    full_rewards_test_df.to_csv(f'../analysis/{str(chicken_game)}/bbl_full_test.csv')
    full_rewards_test_changers_df.to_csv(f'../analysis/{str(chicken_game)}/bbl_full_test_changers.csv')
...
if create_graphs:
    for opponent_key in opponents.keys():
        ...
        plt.savefig(f'../simulations/{str(chicken_game)}/bbl_vs_{opponent_key}.png', bbox_inches='tight')
        plt.clf()
