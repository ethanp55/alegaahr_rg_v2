from generalized.games.prisoner_game import PrisonersGameMDP, baselines
...
data_dir = '../training/training_data/' + str(prisoners_game) + '/'
...
for epoch in range(1, n_epochs + 1):
    ...
    algaater = Algaater('Algaater', prisoners_game, algaater_idx, baselines)
    bbl = BBL('BBL', prisoners_game, bbl_idx)
...
compressed_rewards_df.to_csv(f'../analysis/{str(prisoners_game)}/algaater_vs_bbl_algaater.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(prisoners_game)}/algaater_vs_bbl_bbl.csv')
...
plt.title('AlgAATer vs. BBL - Prisoners\' Dilemma')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(prisoners_game)}/algaater_vs_bbl.png', bbox_inches='tight')
plt.clf()
