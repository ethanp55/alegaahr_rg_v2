...
for epoch in range(1, n_epochs + 1):
    ...
    for round_num in range(n_rounds):
        ...
        key_agent_map = {algaater.name: algaater, bbl.name: bbl} if algaater_idx == P1 else \
            {bbl.name: bbl, algaater.name: algaater}
        ...
        epoch_rewards_1.append(reward_map[algaater.name] / 100)
        prev_rewards_2.append(prev_reward_2)
        epoch_rewards_2.append(reward_map[bbl.name] / 100)
        ...
        print('Agent 1: ' + str(algaater.expert_to_use.name))
        print('Agent 2: ' + str(bbl.expert_to_use.name))
...
compressed_rewards_df.to_csv(f'../analysis/{str(block_game)}/algaater_vs_bbl_algaater.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(block_game)}/algaater_vs_bbl_bbl.csv')
...
plt.title('AlgAATer vs. BBL - Block Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(block_game)}/algaater_vs_bbl.png', bbox_inches='tight')
plt.clf()

print(f'AlgAATer 20 rounds: {mean_test_results[19]}')
print(f'BBL 20 rounds: {mean_opponent_results[19]}')
