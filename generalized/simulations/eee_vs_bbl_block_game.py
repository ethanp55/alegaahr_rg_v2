...
for epoch in range(1, n_epochs + 1):
    ...
    for round_num in range(n_rounds):
        ...
        epoch_rewards_1.append(reward_map[eee.name] / 100)
...
compressed_rewards_df.to_csv(f'../analysis/{str(block_game)}/eee_vs_bbl_eee.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(block_game)}/eee_vs_bbl_bbl.csv')
...
plt.title('EEE vs. BBL - Block Game')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(block_game)}/eee_vs_bbl.png', bbox_inches='tight')
plt.clf()

print(f'EEE 20 rounds: {mean_test_results[19]}')
