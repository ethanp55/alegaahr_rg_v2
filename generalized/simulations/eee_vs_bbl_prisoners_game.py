...
compressed_rewards_df.to_csv(f'../analysis/{str(prisoners_game)}/eee_vs_bbl_eee.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../analysis/{str(prisoners_game)}/eee_vs_bbl_bbl.csv')
...
plt.title('EEE vs. BBL - Prisoners\' Dilemma')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(prisoners_game)}/eee_vs_bbl.png', bbox_inches='tight')
plt.clf()
