...
for epoch in range(1, n_epochs + 1):
    ...
    eee_experts = Algaater.create_aat_experts(coordination_game, eee_idx)
...
if save_data:
    ...
    compressed_rewards_df.to_csv(f'../analysis/{str(coordination_game)}/eee_compressed.csv')
    compressed_rewards_test_df.to_csv(f'../analysis/{str(coordination_game)}/eee_compressed_test.csv')
    compressed_rewards_test_changers_df.to_csv(f'../analysis/{str(coordination_game)}/eee_compressed_test_changers.csv')
    full_rewards_df.to_csv(f'../analysis/{str(coordination_game)}/eee_full.csv')
    full_rewards_test_df.to_csv(f'../analysis/{str(coordination_game)}/eee_full_test.csv')
    full_rewards_test_changers_df.to_csv(f'../analysis/{str(coordination_game)}/eee_full_test_changers.csv')
...
if create_graphs:
    for opponent_key in opponents.keys():
        ...
        plt.savefig(f'../simulations/{str(coordination_game)}/eee_vs_{opponent_key}.png', bbox_inches='tight')
        plt.clf()
