from generalized.games.pennies_game import PenniesGameMDP
from generalized.agents.pennies_game_specific_agents import Random, GreedyUntilNegative, \
...
def create_opponent_agents(player_idx):
    initial_state = pennies_game.get_init_state()
    game_name = str(pennies_game)
    ...
    coop_punish_agent = FolkEgalPunishAgent('CoopPunishOpp', coop_agent, game_name, pennies_game)
    ...
    bully_punish_agent = FolkEgalPunishAgent('BullyPunishOpp', bully_agent, game_name, pennies_game)
    ...
    eee_experts = Algaater.create_aat_experts(pennies_game, player_idx)
    spp = SPP('RoundNum3S++', pennies_game, player_idx)
...
for epoch in range(1, n_epochs + 1):
    ...
    opponent_idx = 1 - bbl_idx

    opponents = create_opponent_agents(opponent_idx)
    bbl = BBL('BBL', pennies_game, bbl_idx, n_samples=10)
    ...
    for opponent_key in opponents.keys():
        ...
        eee_rewards = []
        opp_rewards = []
        reward_map = {opponent_key: 0, bbl.name: 0}
        ...
        for round_num in range(n_rounds):
            ...
            key_agent_map = {bbl.name: bbl, opponent_key: opponent_agent} if bbl_idx == P1 else \
                {opponent_key: opponent_agent, bbl.name: bbl}
            ...
            agent_reward = reward_map[bbl.name]
            eee_rewards.append(agent_reward)
            opp_rewards.append(reward_map[opponent_key])
            bbl.record_opp_actions(round_num, opp_actions)
...
if save_data:
    ...
    compressed_rewards_test_df = pd.DataFrame(vals_test, columns=['BBL'])
    compressed_rewards_test_changers_df = pd.DataFrame(vals_test_changers, columns=['BBL'])
    ...
    compressed_rewards_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_compressed.csv')
    compressed_rewards_test_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_compressed_test.csv')
    compressed_rewards_test_changers_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_compressed_test_changers.csv')
    full_rewards_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_full.csv')
    full_rewards_test_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_full_test.csv')
    full_rewards_test_changers_df.to_csv(f'../analysis/{str(pennies_game)}/bbl_full_test_changers.csv')
...
if create_graphs:
    for opponent_key in opponents.keys():
        ...
        plt.plot(x_vals, algaater_mean_rewards, label='BBL')
        plt.plot(x_vals, opp_mean_rewards, color='red', label=opponent_key)
        plt.title('BBL vs. ' + str(opponent_key))
        plt.xlabel('Round #')
        plt.ylabel('Rewards ($)')
        plt.legend(loc="upper left")
        plt.savefig(f'../simulations/{str(pennies_game)}/bbl_vs_{opponent_key}.png', bbox_inches='tight')
        plt.clf()
